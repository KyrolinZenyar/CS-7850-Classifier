...
namespace Accord.MachineLearning.DecisionTrees
{
    ...
    {
        public static void CheckArgs(ModifiedDecisionTree tree, int[][] inputs, int[] outputs, double[] weights = null)
        {
        ...
        public static void CheckArgs(ModifiedDecisionTree tree, double[][] inputs, int[] outputs, double[] weights = null)
        {
            checkArgs(tree, inputs, outputs, weights);
        ...
        private static void checkArgs(ModifiedDecisionTree tree, Array[] inputs, int[] outputs, double[] weights = null)
        {
            if (inputs == null)
        ...
        public static ModifiedDecisionTree Create(double[][] x, int[] y, IList<ModifiedDecisionVariable> attributes)
        {
            if (attributes == null || attributes.Count == 0)
                attributes = ModifiedDecisionVariable.FromData(x);
            return Create(y, attributes);
        }

        public static ModifiedDecisionTree Create(int[][] x, int[] y, IList<ModifiedDecisionVariable> attributes)
        {
            if (attributes == null || attributes.Count == 0)
                attributes = ModifiedDecisionVariable.FromData(x);
            return Create(y, attributes);
        ...
        private static ModifiedDecisionTree Create(int[] y, IList<ModifiedDecisionVariable> attributes)
        {
            int classes = y.Max() + 1;
            var tree = new ModifiedDecisionTree(attributes, classes);
            return tree;
        }
    }
}
